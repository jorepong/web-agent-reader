...
## 4. 알려진 개선 필요 사항 (구현 예정)

### 4-1. 탐색 에이전트 페이지 마크다운 토큰 효율화 ⚠️ 우선순위 높음

**문제**: explorer가 받는 페이지 마크다운이 너무 큼.
- 실측 예시: ESPN 페이지 43,319자 → 12,307 input 토큰 (전체 비용의 75%)
- 원인: 대형 사이트의 네비게이션 메뉴, UI 컨트롤, 무관한 기사 등이 포함됨

**해결 방향**:
1. `## Navigation`, `## Footer`, `## Aside` 섹션 제거 → Main Content만 추출
2. `[button#BN: ...]`, `[input: ...]`, `[select#SN: ...]` 등 UI 컨트롤 제거
3. 그래도 너무 길면 글자 수 cap 적용 (안전장치)

**주의**: `[LN]` 링크 ID는 제거하면 안 됨. Phase 2 재귀 탐색에서 탐색 에이전트가 어떤 링크로 내려갈지 결정할 때 반드시 필요함.

### 4-2. Excerpts 노이즈

**문제**: 탐색 에이전트가 반환하는 relevantExcerpts에 링크 ID(`[L473]`)나 마크다운 헤딩(`## ...`)이 포함되는 경우 있음.

**해결 방향**: explorer 프롬프트에 "excerpt에서 링크 ID와 마크다운 헤딩 제외" 명시
